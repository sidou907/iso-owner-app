...
# ================= 🔔 إشعارات Web Push (آيفون) =================
VAPID_PUBLIC_KEY = "BBT-PPkhe1sjf-bZ5-i61ESokfjN2Kx9MPxj5jqp1SNcGoavJjFueJ9Np2tVPYZP2b-qmCVWvsy4y8yvlLCR6f0"
...
# الاشتراك ينتهي بإعادة تحميل الصفحة مع بارامترات في الرابط (لا يوجد سيرفر API وسيط، فقط Streamlit نفسه)
_qp = st.query_params
if "push_endpoint" in _qp:
    try:
        save_push_subscription(_qp["push_endpoint"], _qp["push_p256dh"], _qp["push_auth"])
        st.toast("✅ تم تفعيل الإشعارات بنجاح!")
    except Exception as _e:
        st.toast(f"⚠️ فشل تفعيل الإشعارات: {_e}")
